...
top_diffdock_benchmark <- benchmark %>% nest(-ligand, -pdb_file, -affinity, -replicate) %>% 
  ...
  mutate(top_data = purrr::map2(data, top, ~ .x %>% 
                                  # pick the top n most confident poses
                                   arrange(desc(diffdock_confidence)) %>% 
                                   head(min(nrow(.x), .y)) %>%
...
top_smina_benchmark <- benchmark %>% nest(-ligand, -pdb_file, -affinity, -replicate) %>% 
  expand_grid(., top = c(1:100)) %>%
  mutate(top_data = purrr::map2(data, top, ~ .x %>% 
                                  # pick the top n lowest scored poses
...
cor_top_smina_benchmark %>% 
  ...
  ggplot(aes(top, score, color = metric)) + 
  geom_line() + 
...
top_smina_benchmark %>% 
  filter(top == 1) %>% 
  unnest(top_data) %>% 
  ggplot(aes(smina_scored_affinity, affinity, color = diffdock_confidence)) + 
  geom_point() + 
  scale_y_log10() + 
  geom_smooth(method = "lm") + 
  facet_grid(~ replicate) + 
...
top_smina_benchmark %>% 
  filter(top == 1) %>% 
  unnest(top_data) %>% 
  ggplot(aes(diffdock_confidence, smina_scored_affinity, color = diffdock_confidence)) + 
  geom_point() + 
 # scale_y_log10() + 
  geom_smooth(method = "lm") + 
  facet_grid(~ replicate) + 
...
```{r}
winner <- read_csv(here("data/external/d3r_benchmark/submissions/BACEStage1a/structure_based/Structure-Based_Score_Iorga_BestScore_Phase_1a/LigandScores.csv"), 
    skip = 3) %>% 
  dplyr::select(ligand_id = `#LigandID`, winner_score = Score, rank = Rank) %>%
  left_join(bace)



winner %>% 
  ggplot(aes(winner_score, affinity, color = rank)) +
  geom_point() + 
  scale_y_log10() + 
  geom_smooth(method = "lm") + scale_color_viridis_b() + theme_bw()

```

```{r}
cor(winner$winner_score, winner$affinity, method = "spearman")
```
...
top_smina_benchmark %>% 
  ...
  unnest(top_data) %>% 
  left_join(winner) %>% 
  ggplot(aes(winner_score, affinity, color = diffdock_confidence)) + 
  geom_point() + 
  scale_y_log10() + 
  geom_smooth(method = "lm") + 
  facet_grid(~ replicate) + 
...
top_smina_benchmark %>% 
  ...
  unnest(top_data) %>% 
  left_join(winner) %>% 
  ggplot(aes(smina_scored_affinity, winner_score, color = log(affinity))) + 
  geom_point() + 
  #scale_y_log10() + 
  geom_smooth(method = "lm") + 
  facet_grid(~ replicate) + 
